...
public class DLDHDTList<E> extends AbstractDLList<E> {
	...
	public Node<E> getNodeAfter(Node<E> target)
		...
		if(t.getNext()==trailer || header.getNext()==trailer)
			throw new NoSuchElementException("fff");
		return t.getNext(); 
	}
	...
	public Node<E> getNodeBefore(Node<E> target)
		...
		// ADD CODE HERE AND MODIFY RETURN ACCORDINGLY
		DNode<E> t = (DNode<E>)target;
		if(header.getNext()==trailer || length<=1)
			throw new NoSuchElementException("rrftr");
	...
	public void removeNode(Node<E> target) {
		...
		DNode<E> t = (DNode<E>) target;
		DNode<E>pre = t.getPrev();
		DNode<E>aft = t.getNext();
		pre.setNext(aft);
		aft.setPrev(pre);
		t.clean();
		t=null;
